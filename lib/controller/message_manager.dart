...
class MessageManager extends StatelessWidget {
  final Widget child;
  final MessageHandlerController _controller = MessageHandlerController.instance;
  ...
  Future<void> _showAlertDialog(BuildContext context, NotificationMessage notification, MessageHandlerController controller) {
  ...
  Future<void> _showSnackBar(BuildContext context, NotificationMessage notification) {
  ...
  Future<void> _showBottomSheet(BuildContext context, NotificationMessage notification) {
  ...
  Future<void> _showPersistentSnackBar(BuildContext context, NotificationMessage notification) {
    final persistentSnackBar = this.persistentSnackBar ?? const NotificationPersistentSnackBar();
    return ScaffoldMessenger.of(context).showSnackBar(
      persistentSnackBar.buildNotification(context, notification) as SnackBar
    ).closed.then((_) async{
      if(notification.onDismiss != null) {
        await notification.onDismiss!();
      }
